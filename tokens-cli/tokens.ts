 ...
 * Set VAULT_BRIDGE_BASE_URL to your deployed Worker origin so printed URLs are correct.
 */
...
// Base URL of your deployed Worker — used only for the URLs printed below.
// e.g. https://vault-bridge.example.com or https://vault-bridge-relay.<subdomain>.workers.dev
const BASE_URL = (
  process.env.VAULT_BRIDGE_BASE_URL || "https://<your-worker-hostname>"
).replace(/\/+$/, "");
...
function add(label: string, mode: "relay" | "r2" = "relay"): void {
  ...
  console.log(`  MCP URL: ${BASE_URL}/mcp?token=${token}`);
  console.log(
    `  (Claude connector: add ${BASE_URL}/mcp and paste the token on the OAuth consent page)`
  );
  if (mode === "relay") {
    console.log(`  WS URL:  ${WS_BASE_URL}/ws?token=${token}`);
  } else {
  ...
  default:
    console.log(
      "Vault Bridge Token Manager\n\n" +
        ...
        "Example:\n" +
        '  npx tsx tokens-cli/tokens.ts add "alice"\n' +
