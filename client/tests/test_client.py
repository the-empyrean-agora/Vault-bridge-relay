...
@pytest.mark.asyncio
async def test_client_handles_move_file(vault):
    # Uses the raw MCP param names `from`/`to` — exercises the dispatch remap
    # (`from` is a Python keyword and can't pass through **params).
    relay = MockRelay()
    relay.queue_tool_call(
        "move_file", {"from": "notes/hello.md", "to": "archive/hello.md"}
    )
    ...
    assert relay.results[0]["error"] is None
    assert "Moved" in relay.results[0]["result"]
    assert not (vault / "notes" / "hello.md").exists()
    assert (vault / "archive" / "hello.md").read_text() == "Hello from vault"
