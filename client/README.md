...
Connect your Obsidian vault to a Vault Bridge relay so Claude.ai can read, write, and search your notes.
...
- Relay URL (your deployment's WebSocket endpoint, e.g. `wss://<your-worker-hostname>/ws`; can also be set via the `VAULT_BRIDGE_RELAY_URL` env var)
